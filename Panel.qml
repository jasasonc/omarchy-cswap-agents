...
Panel {
  ...
  function canSwitch(p) {
    return !!p && Number(p.cswapNumber || 0) > 0 && !p.cswapActive
  }
  ...
  function heroMeta(p) {
    if (!p) return ""
    var suffix = p.cswapActive ? " · active" : ""
    if (String(p.usageStatusText || "") !== "") return p.usageStatusText + suffix
    var tier = String(p.tierLabel || "")
    var meta = tier === "" ? "Subscription" : tier.charAt(0).toUpperCase() + tier.slice(1)
    return p.cswapActive ? meta + " · active" : meta
  }
  ...
  function iconCandidatesForProvider(p, surfaceColor) {
    if (!p) return []
    // claude-swap account tabs use the Claude mark.
    var markId = String(p.providerId).indexOf("cswap-") === 0 ? "claude" : p.providerId
    var candidates = []
    if (colorLuminance(surfaceColor || Color.background) >= 0.5)
      candidates.push(Qt.resolvedUrl("assets/" + markId + "-light.svg"))
    candidates.push(Qt.resolvedUrl("assets/" + markId + ".svg"))
    return candidates
  }
